...
module ThomasUtils
  describe Future do
    ...
    [nil, ::Concurrent::CachedThreadPool.new].each do |executor|
      context "when the executor is #{executor || 'the default'}" do
        let(:future_wrapper) { Future.new(executor: executor, &block) }

        before do
          expect(::Concurrent::Future).to receive(:execute).with(executor: executor || Future::DEFAULT_EXECUTOR) do |&block|
